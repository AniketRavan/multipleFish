...
def iterateFish_parallel(bKeys, bBoxes, s1Keys, s1Boxes, s2Keys, s2Boxes, triTresh, lut_a_r, pool):
    bFish = []
    s1Fish = []
    s2Fish = []
    bFishBox = []
    ...
    iteratedKeys = np.array(list(itertools.product(bKeys, s1Keys, s2Keys)))
    bKeys_iterated = np.round(np.mean(iteratedKeys[:, 0, 10:12, :], axis=1)).astype(int)
    bKeys_iterated[bKeys_iterated[:,0] == 648, 0] = 647
    bKeys_iterated[bKeys_iterated[:,1] == 488, 1] = 487

    s1Keys_iterated = np.round(np.mean(iteratedKeys[:, 1, 10:12, :], axis=1)).astype(int)
    s1Keys_iterated[s1Keys_iterated[:,0] == 648, 0] = 647
    s1Keys_iterated[s1Keys_iterated[:,1] == 488, 1] = 487

    s2Keys_iterated = np.round(np.mean(iteratedKeys[:, 2, 10:12, :], axis=1)).astype(int)
    s2Keys_iterated[s2Keys_iterated[:,0] == 648, 0] = 647
    s2Keys_iterated[s2Keys_iterated[:,1] == 488, 1] = 487

    b_r = lut_a_r['b_r'][0,0][bKeys_iterated[:, 0].astype(int), bKeys_iterated[:, 1].astype(int)]
    b_a = lut_a_r['b_a'][0,0][bKeys_iterated[:, 0].astype(int), bKeys_iterated[:, 1].astype(int)]
    s1_r = lut_a_r['s1_r'][0,0][s1Keys_iterated[:, 0].astype(int), s1Keys_iterated[:, 1].astype(int)]
    s1_a = lut_a_r['s1_a'][0,0][s1Keys_iterated[:, 0].astype(int), s1Keys_iterated[:, 1].astype(int)]
    s2_r = lut_a_r['s2_r'][0,0][s2Keys_iterated[:, 0].astype(int), s2Keys_iterated[:, 1].astype(int)]
    s2_a = lut_a_r['s2_a'][0,0][s2Keys_iterated[:, 0].astype(int), s2Keys_iterated[:, 1].astype(int)]
    #pdb.set_trace()
    lossArray = pool.starmap(triangulate_using_rays, zip(b_r, b_a, s1_r, s1_a, s2_r, s2_a))
    idx = 0
    iteratedKeys = list(itertools.product(bKeys, s1Keys, s2Keys))
...
def return_YOLO_output(model, rgbImage, lut_a_r, pool):
    ...
    for cls in classes:
        shift = 10
        boxes[idx, 0:2] = np.maximum(boxes[idx, 0:2] + np.array([-shift, -shift]), np.array([0, 0]))
        boxes[idx, 2:4] = np.minimum(boxes[idx, 2:4] + np.array([shift, shift]), np.array([648, 488]))
        if cls == 0.0 :
    ...
    triTresh = 1
    bFish, bFishBox, s1Fish, s1FishBox, s2Fish, s2FishBox = iterateFish_parallel(np.array(bKeys), np.array(bBoxes), np.array(s1Keys), np.array(s1Boxes), \
            np.array(s2Keys), np.array(s2Boxes), triTresh, lut_a_r, pool)
    return bFish, bFishBox, s1Fish, s1FishBox, s2Fish, s2FishBox
