...
def calc_proj_w_refra_cpu(coor_3d,proj_params = 'proj_params_101019_corrected_new.mat' ):
...
def return_head_real_model_new(x,fishlen,proj_params,cb,cs1,cs2):
    ...
    # S1 upperparts, try except blocks kept seperate in case one is drawable
    try:
        eye_scaling = max(eye_br / float(max(eye1_s1.max(axis=0))), eye_br / float(max(eye2_s1.max(axis=0))))
    except ZeroDivisionError:
        #The eye was out of bounds, the eye scalling does not matter
        eye_scaling = 1
    ...
    try:
        ...
        #Out of bounds, eye scalling dosent matter since it is out of the picture
        eye_scaling = 1
...
def view_s_lut_new_real_cpu(crop_coor, pt,lut_tail,projection,imageSizeX,imageSizeY):
    ...
    for ni in range(0,7):
        ...
        try:
            ...
            #Tail went out of bounds, so we do not take those values
            throwaway = 99
        bodypix = np.maximum(bodypix, tailpix)
...
def return_graymodels_fish(x,lut_b_tail,lut_s_tail,proj_params,fishlen,imageSizeX,imageSizeY,cropFlag = False,crop_bIn =[0] ,crop_s1In = [0] ,crop_s2In = [0]):
...
def isFishWithinBounds(fishX, fishlen):
    ...
    for x in range(6):
        ...
        # For reference as to whether they represent rows or columns
...
def createDepthArr(img, xIdx, yIdx, d):
    ...
    # cut out not necessary but good for visualizing
    depthArrCutOut = np.zeros(img.shape)

    try:
        depthArr[(np.ceil(xIdx).astype(int)), (np.ceil(yIdx).astype(int))] = d
    except IndexError:
        throwaway = 'was out of bounds'


    # TODO replace with some type of convolution to speed up if necessary
    radius = 14
    for point in range(10):
        [backboneX, backBoneY] = [(np.ceil(xIdx).astype(int))[point], (np.ceil(yIdx).astype(int))[point]]
        depth = d[point]
        for n in range(1, radius):
            maxX = n
            for varY in range(-n, n + 1):
                try:
                    depthArr[backboneX + maxX, backBoneY + varY] = depth
                except:
                    throwaway = 'temporary solution :)'
                try:
                    depthArr[backboneX - maxX, backBoneY + varY] = depth
                except:
                    throwaway = 'temporary solution :)'
            maxY = n
            for varX in range(-n, n + 1):
                try:
                    depthArr[backboneX + varX, backBoneY + maxY] = depth
                except:
                    throwaway = 'temporary solution :)'
                try:
                    depthArr[backboneX + varX, backBoneY - maxY] = depth
