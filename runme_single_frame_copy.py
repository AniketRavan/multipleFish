...
import time
from pose_prediction_helper_functions_copy import *
import cv2
...
try:
    for videoFilePath, dataTime in zip(videoFilePaths, dataTimeArray):
        ...
        for i in range(0, bgsubVidB.shape[0]):
            ...
            bFish, bFishBox, s1Fish, s1FishBox, s2Fish, s2FishBox = return_YOLO_output(yolo_model, frame, proj_params, pool)
            ...
            eyes_b_iterated = pose_recon_b[:, :, iter_idx_eyes[:, 0]]
            eyes_s1_iterated = pose_recon_s1[:, :, iter_idx_eyes[:, 1]]
            eyes_s2_iterated = pose_recon_s2[:, :, iter_idx_eyes[:, 2]]
            ...
            eye1_3d = eyes_3d[:, :, first_eye_minimum_idx]
            eye2_3d = eyes_3d[:, :, -first_eye_minimum_idx] # Negative sign to capture alternate combinations
            pose_3d = np.zeros((nFish, 3, 12))
            pose_3d[:, :, 0:10] = reconstructed_3d_backbone
            pose_3d[:, :, 10] = eye1_3d[:, :, 0] # Don't worry about 0: only one element in this dim
            pose_3d[:, :, 11] = eye2_3d[:, :, 0] # Don't worry about 0: only one element in this dim
            ...
            for fishIdx in range(nFish):
                ...
                img = imResnet[fishIdx, :, :, :]
                p_b = pose_projection_b[fishIdx] - bFishBox[fishIdx, [0, 1], None] + padding_offset[fishIdx, 0, :, None]
                p_s1 = pose_projection_s1[fishIdx] - s1FishBox[fishIdx, [0, 1], None] + padding_offset[fishIdx, 1, :, None]
                p_s2 = pose_projection_s2[fishIdx] - s2FishBox[fishIdx, [0, 1], None] + padding_offset[fishIdx, 2, :, None]
